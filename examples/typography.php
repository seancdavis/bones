<?php include 'partials/header.php' ?>

<div class="container">
  
  <h1>Heading 1</h1>
  <h2>Heading 2</h2>
  <h3>Heading 3</h3>
  <h4>Heading 4</h4>
  <h5>Heading 5</h5>
  <h6>Heading 6</h6>
  
  <hr>
  
  <h1 class="subheading">Subheading 1</h1>
  <h2 class="subheading">Subheading 2</h2>
  <h3 class="subheading">Subheading 3</h3>
  <h4 class="subheading">Subheading 4</h4>
  <h5 class="subheading">Subheading 5</h5>
  <h6 class="subheading">Subheading 6</h6>
  
  <hr>
  
  <h1>Heading 1 <small>with a small addition</small></h1>
  <h2>Heading 2 <small>with a small addition</small></h2>
  <h3>Heading 3 <small>with a small addition</small></h3>
  <h4>Heading 4 <small>with a small addition</small></h4>
  <h5>Heading 5 <small>with a small addition</small></h5>
  <h6>Heading 6 <small>with a small addition</small></h6>
  
  <hr>
  
  <p>Bacon ipsum dolor sit amet pork belly ribeye pork loin andouille frankfurter leberkas venison swine corned beef shoulder. T-bone hamburger turducken brisket ground round ball tip tongue meatball flank chuck kielbasa corned beef cow tri-tip short ribs. Meatball hamburger corned beef pig. Pancetta bresaola beef ribs, short loin beef frankfurter jowl drumstick fatback sausage short ribs turkey rump jerky t-bone.</p>
  
  <p>
    <a href="#">Links</a> are configurable, including their hover behavior. In-paragraph text can also be <big>big</big>, <small>small</small>, <em>italicized</em> or <strong>bold</strong>.
  </p>
  
  <hr>
  
  <blockquote>
    <p>Blockquotes are a great way to call out text.</p>
    <p>Not the only way, but a great way.</p>
  </blockquote>
  
  <hr>

  <p>Code, like <code>$this_variable</code>, can be embedded in any paragraph.</p>
  
  <hr>

<pre>
function sayHello() {
  alert "Hello World!";
}
</pre>

<hr>
  
  <ul>
    <li>Item 1</li>
    <li>Item 2
      <ul>
        <li>Item a</li>
        <li>Item b
          <ul>
            <li>Item i</li>
            <li>Item ii</li>
          </ul>
        </li>
      </ul>
    </li>
  </ul>
  
  <hr>
  
  <ol>
    <li>Item 1</li>
    <li>Item 2
      <ol>
        <li>Item a</li>
        <li>Item b
          <ol>
            <li>Item i</li>
            <li>Item ii</li>
          </ol>
        </li>
      </ol>
    </li>
  </ol>
  
  <hr>
  
  <ul class="horizontal">
    <li>Item 1</li>
    <li>Item 2</li>
    <li>Item 3</li>
    <li>Item 4</li>
  </ul>
  
  <hr>
  
  <ul class="no-bullet">
    <li>Item 1</li>
    <li>Item 2</li>
    <li>Item 3</li>
    <li>Item 4</li>
  </ul>
  
  <br><br>
  
</div>

<?php include 'partials/footer.php' ?>